...
int main(void)
{
    ...
    int nombreManches; // Le nombre de manche de la partie
    int mancheEnCours = 1; // Le numéro de la manche actuel de la partie en cours
    ...
    afficherTexteEnCouleur("Bienvenue dans le jeu du Chifoumi !", cyan, true);
    cout << " " << endl;
    afficherTexteEnCouleur("Les regles sont simples : choisi un coup entre la pierre, la feuille ou le ciseau", cyan, true);
    afficherTexteEnCouleur("La pierre gagne contre le ciseau, qui gagne contre la feuille qui gagne contre la pierre", cyan, true);
    cout << " " << endl;
    afficherTexteEnCouleur("La partie ne s'arrete jamais : appuye sur 'Q' a la fin de la manche pour arreter la partie.", cyan, true);
    ...
    cout << "Veuillez rentrer le nombre de manche de la partie -- > ";
    cin >> nombreManches;

    for(mancheEnCours; mancheEnCours <= nombreManches; mancheEnCours++)
    {
        // affichageNouvelleManche >> écran
        afficherTexteEnCouleur("Manche numero ", cyan, false);
        afficherNombreEnCouleur(mancheEnCours, cyan, true);
        afficherTexteEnCouleur("Chifoumi !", cyan, true);
        cout << " " << endl;
        afficherTexteEnCouleur("Veuillez saisir le coup que vous souhaitez jouer : ", cyan, true);

        // clavier >> saisieCoupJoueur >> coupJoueur
        while (true)
        {
            afficherTexteEnCouleur("(P)ierre ? (F)euille ? ou (C)iseau ?", cyan, false);
            cin >> coupJoueur;

            if ((coupJoueur == 'P' || coupJoueur == 'F' || coupJoueur == 'C'))
            {
                break;
            }
            else{
                afficherTexteEnCouleur("ERREUR : coup inconnu", rouge, true);
            }
        }
        ...
        if (gagnantManche == 'e')
        {
            afficherTexteEnCouleur("Egalite. Il n'y a pas de gagnant pour cette manche.", gris, true);
        }
        ...
        afficherTexteEnCouleur("    /    ", gris, false);
    ...
    {
        afficherTexteEnCouleur("Egalite. Il n'y a pas de gagnant pour cette partie.", gris, true);
    }
    ...
    //Saisie servant à que l'invite de commande ne se ferme pas lors de l'execution du fichier en .exe
    cout << "Appuyez sur une touche puis entrer pour quitter le jeu " << endl;
    cin >> affichageCoup;
    return 0;
}
